...
from SimComponents_master_plan import DataframeSource, Sink, Process
import time
...
def gen_block_data(block_data):
    for i in range(len(block_data)):
        ...
        proj_no = project['PROJECTNO'][0]
        location_code = project['LOCATIONCODE'][0]
        dict_activity = OrderedDict()
        for j in range(len(project)):
            temp = project.iloc[j]
            dict_activity[temp['ACTIVITY']] = [temp['PLANSTARTDATE'], temp['PLANDURATION']]

        yield [proj_no, location_code, dict_activity]


b = gen_block_data(block2)
...
process_dict = {}
Source = DataframeSource(env, "Source", b, process_dict, len(block2))
...
for proj_no in result.keys():
    for location_code in result[proj_no].keys():
        temp_dict = {}
